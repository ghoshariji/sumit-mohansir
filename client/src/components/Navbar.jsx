...
import {
  FaSearch, FaMoon, FaSun, FaShoppingCart, FaGraduationCap, FaCode,
  FaMagic, FaChevronLeft, FaChevronRight, FaBars, FaTimes
} from 'react-icons/fa';
import API from '../api';
...
<nav className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-6 py-2 bg-white dark:bg-gray-900 shadow dark:text-white">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden md:flex items-center gap-2">
            ...
            <div className="flex items-center gap-6 text-sm ml-6">
              <span className="flex items-center gap-1 cursor-pointer"><FaMagic /> Plus</span>
              <span className="flex items-center gap-1 cursor-pointer"><FaCode /> Spaces</span>
              <span className="flex items-center gap-1 cursor-pointer"><FaGraduationCap /> For Teachers</span>
              <span className="flex items-center gap-1 cursor-pointer"><FaShoppingCart /> Get Certified</span>
              <button onClick={() => navigate('/register')} className="bg-green-500 text-white px-4 py-1 rounded-full hover:bg-green-600">Register</button>
              <button onClick={() => navigate('/login')} className="bg-green-100 text-green-800 px-4 py-1 rounded-full hover:bg-green-200">Log in</button>
            </div>
          </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white dark:bg-gray-800 text-black dark:text-white px-4 py-2 space-y-2">
          ...
          <hr />
          <span className="flex items-center gap-2"><FaMagic /> Plus</span>
          <span className="flex items-center gap-2"><FaCode /> Spaces</span>
          <span className="flex items-center gap-2"><FaGraduationCap /> For Teachers</span>
          <span className="flex items-center gap-2"><FaShoppingCart /> Get Certified</span>
  ...
  <div className="sticky top-[56px] z-40 bg-gray-900 text-white py-2">
          ...
          <div ref={coursesRef} className="overflow-x-auto whitespace-nowrap scroll-smooth px-10 hide-scrollbar">
            {languages.map((lang) => (
              <Link
                key={lang._id}
                to={`/course/${lang.programmingLanguage}/title/${lang.defaultContent || 'intro'}`}
          ...
          <button onClick={scrollRight} className="absolute right-0 top-1/2 transform -translate-y-1/2 p-2 z-10">
            <FaChevronRight />
          </button>
        </div>
      )}
